...
describe("Book Model Tests", () => {
  beforeEach(() => {
    ...
    // モックを注入
    setBookModelInstance(mockBookModel);

    // デフォルトのモックを設定 - 常にsuccess形式で返す
    mockBookModel.create.mockResolvedValue({ success: true, data: null });
    ...
    mockBookModel.update.mockResolvedValue({ success: true, data: null });
    mockBookModel.delete.mockResolvedValue({ success: true, data: null });
    mockBookModel.deleteAll.mockResolvedValue({ success: true, data: null });
  });
  ...
  describe("createBook", () => {
    test("有効な書籍情報で書籍作成が成功する", async () => {
      ...
      // モックの設定
      mockBookModel.create.mockResolvedValue({ success: true, data: { isbn } });
    ...
    test("空のisbnでエラーが発生する", async () => {
      ...
      await expect(createBook(isbn, title, author)).rejects.toThrow(
        "Cannot empty isbn, title, and author.",
      );
  ...
  describe("getBookByIsbn", () => {
    ...
    test("存在しないISBNで失敗する", async () => {
      ...
      // モックの設定
      mockBookModel.findOne.mockResolvedValue({ success: false, data: null });
    ...
    test("isbnフィールドがない書籍を適切に処理する", async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.data).toBeNull();
      expect(result.message).toBe("書籍データが不正です");
    });
  });
  ...
  describe("getBookByAuthor", () => {
    ...
    test("空の著者名でエラーが発生する", async () => {
      ...
      await expect(getBookByAuthor(author)).rejects.toThrow(
        "Cannot empty author.",
      );
  ...
  describe("updateBook", () => {
    test("有効な書籍情報で更新が成功する", async () => {
      ...
      expect(mockBookModel.update).toHaveBeenCalledWith(isbn, {
        title,
        author,
      });
    });
    ...
    test("空のisbnでエラーが発生する", async () => {
      ...
      await expect(updateBook(isbn, title, author)).rejects.toThrow(
        "Cannot empty isbn, title, and author.",
      );
